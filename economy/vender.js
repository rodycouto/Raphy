...
exports.run = async (client, message, args) => {
    ...
    if (author1 !== null && timeout1 - (Date.now() - author1) > 0) {
        ...
        if (['diamantes', 'diamante', 'diamond'].includes(args[0])) {
            ...
            if (!args[1]) {
                return message.inlineReply('Quantos diamantes você quer vender? `' + prefix + 'vender diamantes quantidade`')
            }
        ...
        if (['maças', 'apple', 'maça', 'apples'].includes(args[0])) {
            ...
            if (!args[1]) {
                return message.inlineReply('Quantas madeiras você quer vender? `' + prefix + 'vender maças quantidade`')
            }
